...
class Monkey:
    def __init__(self, list_of_items, if_true, if_false, throw_condition, worry_update, relief_function = lambda x: x//3):
    ...
    with open(filename) as file:
        while(True):
            try:
                ...
                file.readline()
                list_of_monkeys.append(Monkey(items,pass_true,pass_false,test,worry_function))
            except:
                break
    return list_of_monkeys

if __name__ == '__main__':
    monkeys = MonkeyCollection(parse_input(input_file))
    for i in range(20):
        monkeys.do_round()
